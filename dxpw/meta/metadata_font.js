class MetadataFont {
  ...
  static WREF = 'https://learn.microsoft.com/en-us/typography/opentype/spec/';
  ...
  getTable(r){
    ...
    let tag = table.tag.replace('/','').replace(' ','');
    return this[tag]
      ? this[tag].load(r, table)
      : {
        header: table
      };
  }
  ...
  cmap = {
    ...
    EncodingRecord: function(r, tableOffset){
      let out = {};
      out.platformID = MetadataFont.PLATFORM_ID.readUInt16(r);
  ...
  EBLC = {
    ...
    load: function(r, table){
      ...
      for (let i = 0; i < this.data.numSizes; i++){
        ...
        record.hori = SbitLineMetrics(r);
        record.vert = SbitLineMetrics(r);
  ...
  name = {
    ...
    NameRecord: function(r, stringOffset){
        let record = {};
        record.platformID = MetadataFont.PLATFORM_ID.readUInt16(r); // Platform ID.
        record.encodingID = r.readUInt16();            // Platform-specific encoding ID.
        record.encodingID_desc = MetadataFont.PLATFORM_ID.encodingId(record.platformID, record.encodingID);
        record.languageID = r.readUInt16();            // Language ID.
        record.nameID = MetadataFont.NAME_ID.readInt16(r);          // Name ID.
